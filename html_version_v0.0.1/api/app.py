...
def get_chart():
    ...
    try:
        ...
        # 거래량 추출 (다양한 컬럼명 시도)
        volumes = []
        for col in ['volume', 'Volume', 'vol', 'Vol', 'VOL']:
            if col in df.columns:
                volumes = df[col].tolist()
                print(f"✅ 거래량 컬럼 '{col}' 발견: {len(volumes)}개")
                break
        
        if not volumes:
            print(f"⚠️ 거래량 컬럼을 찾을 수 없습니다. 사용 가능한 컬럼: {df.columns.tolist()}")
        ...
        if volumes:
            for i, price in enumerate(prices):
                if i < len(volumes):
                    trade_amount = price * volumes[i]
                    trade_amounts.append(trade_amount)
        
        current_price = prices[-1] if prices else 0
        current_volume = volumes[-1] if volumes else 0
        current_trade_amount = trade_amounts[-1] if trade_amounts else 0
        
        print(f"✅ [{timeframe}] 차트 데이터 반환: {len(prices)}개 캔들, 현재가={current_price:,.0f} KRW")
        print(f"   가격 범위: 최저={min(prices):,.0f} KRW, 최고={max(prices):,.0f} KRW")
        print(f"   거래량: {len(volumes)}개, 거래대금: {len(trade_amounts)}개")
        if volumes:
            print(f"   최신 거래량: {current_volume:,.0f}, 최신 거래대금: {current_trade_amount:,.0f}")
        
        return jsonify({
            'prices': prices,
            'volumes': volumes,
            'trade_values': trade_amounts,
            'trade_amounts': trade_amounts,
            'timeframe': timeframe,
            'mapped_interval': pyupbit_interval,
            'current_price': current_price,
            'current_volume': current_volume,
            'current_trade_amount': current_trade_amount,
            'timestamp': datetime.now().isoformat()
        })
    except Exception as e:
...
def calculate_nb():
    """N/B 값 계산 (가격, 거래량, 거래대금 지원)"""
    try:
        data = request.json
        chart_data = data.get('chart_data')
        prices = data.get('prices')
        data_type = data.get('data_type', 'price')  # 'price', 'volume', 'trade_amount'
        
        if not prices and chart_data:
            prices = chart_data.get('prices')
        
        if not prices or len(prices) < 2:
            return jsonify({'error': f'{data_type} 데이터가 부족합니다.'}), 400
...
# N/B 및 BIT MAX 검색 API (파일 탐색 기반)
@app.route('/api/nb/search', methods=['GET'])
def search_nb():
    try:
        if not nbverse_storage:
            return jsonify({'error': 'NBVerse가 초기화되지 않았습니다.'}), 500
        
        bit_max_param = request.args.get('bit_max')
        nb_value_param = request.args.get('nb_value')
        tolerance = request.args.get('tolerance', '1e-9')
        max_scan_param = request.args.get('max_scan', '5000')
        try:
            tolerance = float(tolerance)
        except ValueError:
            return jsonify({'error': 'tolerance는 숫자여야 합니다.'}), 400
        try:
            max_scan = int(max_scan_param)
            if max_scan <= 0:
                max_scan = 5000
        except ValueError:
            max_scan = 5000
        ...
        criteria = []
        if bit_max_param is not None:
            try:
                criteria.append(('bit_max', float(bit_max_param)))
            except ValueError:
                return jsonify({'error': 'bit_max는 숫자여야 합니다.'}), 400
        if nb_value_param is not None:
            try:
                criteria.append(('nb_value', float(nb_value_param)))
            except ValueError:
                return jsonify({'error': 'nb_value는 숫자여야 합니다.'}), 400
        
        if not criteria:
            return jsonify({'error': 'bit_max 또는 nb_value 파라미터가 필요합니다.'}), 400
        
        current_file_dir = os.path.dirname(os.path.abspath(__file__))
        parent_dir = os.path.dirname(os.path.dirname(current_file_dir))
        data_dir = os.path.join(parent_dir, "data", "nbverse")
        base_dirs = [os.path.join(data_dir, "max"), os.path.join(data_dir, "min")]
        ...
        for base_dir in base_dirs:
            ...
            for root, dirs, files in os.walk(base_dir):
                for filename in files:
                    ...
                    matched = True
                    for key, target in criteria:
                        meta_val = None
                        if key == 'bit_max':
                            meta_val = metadata.get('bit_max') or data.get('bit_max') or data.get('nb_max')
                        else:
                            meta_val = metadata.get('nb_value') or data.get('nb_value')
                        if meta_val is None:
                            matched = False
                            break
                        try:
                            ...
                            matched = False
                            break
                        if abs(meta_val - target) > tolerance:
                            matched = False
                            break
                    if matched:
                        ...
                        if len(matches) >= 50:
                            break
                if len(matches) >= 50 or limit_reached:
                    break
            if len(matches) >= 50 or limit_reached:
                break
...
# BIT MAX 경로 기반 조회 API (부분 입력도 하위 폴더 전체 검색)
@app.route('/api/nb/path', methods=['GET'])
def get_nb_by_path():
    try:
        if not nbverse_storage:
            return jsonify({'error': 'NBVerse가 초기화되지 않았습니다.'}), 500
        bit_raw = request.args.get('bit_max')
        if not bit_raw:
            return jsonify({'error': 'bit_max 파라미터가 필요합니다.'}), 400
        try:
            bit_val = float(bit_raw)
        except ValueError:
            return jsonify({'error': 'bit_max는 숫자여야 합니다.'}), 400
        prefix = bit_raw.strip()
        if not prefix:
            return jsonify({'error': 'bit_max 파라미터가 비어 있습니다.'}), 400
        max_scan_param = request.args.get('max_scan', '20000')
        try:
            max_scan = int(max_scan_param)
            if max_scan <= 0:
                max_scan = 20000
        ...
        parent_dir = os.path.dirname(os.path.dirname(current_file_dir))
        data_dir = os.path.join(parent_dir, "data", "nbverse")
        base_dir = os.path.join(data_dir, "max")
        if not os.path.exists(base_dir):
            return jsonify({'found': False, 'message': 'max 폴더가 없습니다.', 'folder': os.path.relpath(base_dir, parent_dir)}), 404
        # 가능한 한 폴더 깊이를 따라가되, 남은 구간은 전체 탐색
        digits = list(prefix.replace('.', ''))
        cursor_dir = base_dir
        for d in digits:
            next_dir = os.path.join(cursor_dir, d)
            if os.path.exists(next_dir) and os.path.isdir(next_dir):
                cursor_dir = next_dir
            else:
                break  # 더 내려갈 수 없으면 여기서부터 탐색
        scanned = 0
        found_file = None
        limit_reached = False
        for root, dirs, files in os.walk(cursor_dir):
            for fname in files:
                ...
                if fname.startswith(prefix):
                    found_file = os.path.join(root, fname)
                    break
            if found_file or limit_reached:
                break
        if not found_file:
            return jsonify({
                'found': False,
                'message': '파일을 찾을 수 없습니다.',
                'search_root': os.path.relpath(cursor_dir, data_dir),
                'scanned_files': scanned,
                'limit_reached': limit_reached
            }), 404
        try:
            data = nbverse_storage.load_from_path(found_file)
        except Exception as e:
            return jsonify({'found': False, 'error': str(e), 'file': os.path.relpath(found_file, data_dir)}), 500
        return jsonify({
            'found': True,
            'file': os.path.relpath(found_file, data_dir),
            'data': data,
            'scanned_files': scanned,
            'limit_reached': limit_reached,
            'bit_max_input': prefix,
            'bit_max_normalized': f"{bit_val:.10f}"
        })
    except Exception as e:
        import traceback
        traceback.print_exc()
...
# N/B 값 조회 API (nb_value 기준 단건)
@app.route('/api/nb/<nb_value>', methods=['GET'])
def get_nb(nb_value):
    """N/B 값 조회"""
    try:
        try:
            nb_value_float = float(nb_value)
        except ValueError:
            return jsonify({'error': 'nb_value는 숫자여야 합니다.'}), 400
        ...
        # 단건 조회는 search 엔드포인트를 재사용
        with app.test_request_context(f"/api/nb/search?nb_value={nb_value_float}"):
            resp = search_nb()
            # search_nb는 Response 또는 (json, status) 반환 가능
            return resp
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def sell_card(card_id):
    ...
    try:
        ...
        # 가장 최근 BUY 히스토리에서 진입 가격과 수량 가져오기
        entry_price = 0.0
        qty = 0.0
        ...
        # 여전히 찾지 못한 경우 현재 가격 사용
        if entry_price <= 0:
            entry_price = current_price
            if qty <= 0:
                min_buy_amount = settings_manager.get('min_buy_amount', 5000) if settings_manager else 5000
                qty = min_buy_amount / current_price if current_price > 0 else 0
        
        # 최종 검증
        if entry_price <= 0 or qty <= 0:
            print(f"⚠️ 진입 가격 또는 수량을 찾을 수 없습니다. card_id={card_id}, entry_price={entry_price}, qty={qty}")
            print(f"   히스토리: {[h.get('type') for h in history_list[:5]]}")
            # 기본값 사용 (오류 대신 경고만)
            if entry_price <= 0:
                entry_price = current_price
            if qty <= 0:
                min_buy_amount = settings_manager.get('min_buy_amount', 5000) if settings_manager else 5000
                qty = min_buy_amount / entry_price if entry_price > 0 else 0.0001
            print(f"   기본값 사용: entry_price={entry_price}, qty={qty}")
...
def save_chart_analysis_card():
    ...
    try:
        ...
        # 카드 ID 생성 (요청에 따라 basic_nb 프리픽스 적용)
        # 예: basic_nb_card2_20260105_123456_123456
        card_id = f"basic_nb_{card_type}_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}"
